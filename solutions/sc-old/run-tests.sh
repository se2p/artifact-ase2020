...
BASTET_ROOT="$HOME/uni/develop/bastet-framework"

if [ -z "$BASTET_ROOT" ]
then
    echo "Please define the environment variable BASTET_ROOT"
    exit 1
fi
...
do
    ...
    if [[ $f == *"task9"* ]]
    then
        SPEC_FILE="$TEST_DIR/../../spec/bastet/task9_horse.sc"
    elif [[ $f == *"task8"* ]]
    then
        SPEC_FILE="$TEST_DIR/../../spec/bastet/task8_cat.sc"
    elif [[ $f == *"task7"* ]]
    then
        SPEC_FILE="$TEST_DIR/../../spec/bastet/task7_elephant.sc"
    elif [[ $f == *"task6"* ]]
    then
        SPEC_FILE="$TEST_DIR/../../spec/bastet/task6_monkey.sc"
    fi
